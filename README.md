...
房间必填，具体位置和详细位置可按情况留空。默认常用房间包括：

```text
厨房、客厅、餐厅、书房、卧室、阳台、厕所、洗手台、玄关
```
...
每个位置可以填写别名，例如 `左抽,客厅抽屉,书房货架`。之后语音录入时，AI 会优先从这些已有位置里匹配。比如系统里只有 `书房 / 货架`，你说“放在货架第三层”，AI 会优先推断为 `书房 / 货架 / 第三层`。
